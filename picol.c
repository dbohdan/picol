...
int picolMatch(char* pat, char* str) {
    ...
    if (*pat == '*') {
        ...
        res = picolWildEq(pat,str,-1);      /* <xx> case */
    }
    free(mypat);
    return res;
}
...
picolVar* picolArrSet(picolArray* ap, char* key, char* value) {
    ...
    if (v) {
        free(v->val); /* found exact key match */
...
picolInterp* picolCreateInterp(void) {
    ...
    srand(clock());
    picolArrSet1(i,"::tcl_platform(platform)", TCL_PLATFORM_PLATFORM);
    return i;
}
